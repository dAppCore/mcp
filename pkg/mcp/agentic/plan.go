...
func (s *PrepSubsystem) planCheckpoint(_ context.Context, _ *mcp.CallToolRequest, input PlanCheckpointInput) (*mcp.CallToolResult, PlanCheckpointOutput, error) {
	...
	if input.Phase <= 0 {
		return nil, PlanCheckpointOutput{}, core.E("planCheckpoint", "phase must be greater than zero", nil)
	}
	if input.Notes == "" && !input.Done {
		return nil, PlanCheckpointOutput{}, core.E("planCheckpoint", "notes or done is required", nil)
	}
	...
	if phaseIndex >= len(plan.Phases) {
		return nil, PlanCheckpointOutput{}, core.E("planCheckpoint", "phase not found", nil)
	}
	...
	plan.UpdatedAt = time.Now()
	if _, err := writePlan(s.plansDir(), plan); err != nil {
		return nil, PlanCheckpointOutput{}, core.E("planCheckpoint", "failed to write plan", err)
	}
